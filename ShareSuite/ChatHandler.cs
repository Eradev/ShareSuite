    ...
    public static class ChatHandler
    {
        ...
        private static List<PlayerCharacterMasterController> GetEligiblePlayers(CharacterBody body)
        {
            ...
            foreach (var playerCharacterMasterController in PlayerCharacterMasterController.instances)
            {
                ...
                // If they don't have a body or are the one who picked up the item, go to the next player
                if (!master.inventory || master.GetBody() == body)
                {
                    continue;
                }

                // If the player is alive or DeadPlayersGetItems is enabled, add to eligiblePlayers
